module AndresLoeh where
  ...
  parseStmt_ e =
        do
          ...
          reserved lambdaPi "="
          t <- parseITerm_ 0 e
          return (Let (traceShowId x) (traceShowId t))
    <|> do
          reserved lambdaPi "assume"
          (xs, ts) <- parseBindings_ False [] 
          return (Assume (traceShowId (reverse (zip xs ts))))
    <|> do
    ...
    let rec int state =
          do
            x <- catch
                  ...
                  (\(err :: IOException) -> return Nothing)
            case x of
              Nothing   ->  return ()
              Just ""   ->  rec int state
              Just x    ->
