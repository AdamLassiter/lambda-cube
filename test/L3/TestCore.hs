...
testShowCtx :: IO ()
testShowCtx = do
  assertEq (showCtx (Ctx [] :: Context Int)) "" "Show empty context"
  assertEq (showCtx (Ctx [(1, Star)] :: Context Int)) "\n(1,*)" "Show one-elem context"
...
testInferType = do
  ...
  assertEq (inferType (Ctx [(Name "x", Star)]) (Var (Name "x"))) (Right Star) "x :: * ⊢ x :: *"

  assertEq (inferType0 (Lam (Name "x") Star (Var (Name "x")))) (Right $ Pi (Name "x") Star Star) "lam x : * -> x :: pi x : * -> *"
  assertEq (inferType0 (Lam (Name "x") Star (Var (Name "x")))) (Right $ Pi (Name "x") Star Star) "lam x : * -> x :: pi x : * -> *"

  assertEq (inferType (Ctx [(Name "a", Star)]) (Pi (Name "x") (Var (Name "a")) (Var (Name "a")))) (Right Star) "a :: * ⊢ pi x : a -> a :: *"
  assertEq (inferType0 (Pi (Name "x") Star (Var (Name "x")))) (Right Star) "pi x : * -> x :: *"
  assertEq (inferType (Ctx [(Name "a", Star)]) (Pi (Name "x") (Var (Name "a")) Star)) (Right Box) "a :: * ⊢ pi x : * -> * :: #"
  ...
  assertEq (inferType (Ctx ctx) (Var (Name "map") `App` Var (Name "b") `App` Var (Name "c") `App` Var (Name "f"))) (Right $ Pi (Name "l") (App (Var $ Name "List") (Var $ Name "b")) (App (Var $ Name "List") (Var $ Name "c"))) "map :: pi a -> pi b -> lam (a -> b) -> List a -> List b, f :: b -> c ⊢ map b c f :: List b -> List c"

testWellTyped :: IO ()
testWellTyped = do
  assertTrue (wellTyped0 (Star :: Expr Name)) "* :: #"

  assertTrue (wellTyped (Ctx [(Name "x", Star)]) (Var (Name "x"))) "x :: * ⊢ x :: *"

  assertTrue (wellTyped0 (Lam (Name "x") Star (Var (Name "x")))) "lam x : * -> x :: pi x : * -> *"
  assertTrue (wellTyped0 (Lam (Name "x") Star (Var (Name "x")))) "lam x : * -> x :: pi x : * -> *"

  assertTrue (wellTyped (Ctx [(Name "a", Star)]) (Pi (Name "x") (Var (Name "a")) (Var (Name "a")))) "a :: * ⊢ pi x : a -> a :: *"
  assertTrue (wellTyped0 (Pi (Name "x") Star (Var (Name "x")))) "pi x : * -> x :: *"
  assertTrue (wellTyped (Ctx [(Name "a", Star)]) (Pi (Name "x") (Var (Name "a")) Star)) "a :: * ⊢ pi x : * -> * :: #"
  ...
  assertTrue (wellTyped (Ctx ctx) (Var (Name "map") `App` Var (Name "b") `App` Var (Name "c") `App` Var (Name "f"))) "map :: pi a -> pi b -> lam (a -> b) -> List a -> List b, f :: b -> c ⊢ map b c f :: List b -> List c"
